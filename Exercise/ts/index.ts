...
const renderUI = () => {
  ...
  for (let i = 0; i < Elementli.length; i++) {
    product.forEach((item: { discount: number; imageUrl: string; name: string; price: number; id: number; }) => {
      if (item.discount != 0) {
...
const addToCart = () => {
  let buttonCart = document.querySelectorAll('.btn-cart');
    for (let i = 0; i < buttonCart.length; i++) {
      ...
      changeQuantityItemInCart('add', (e.target as HTMLElement).attributes['data-id'].value);
      countCart();
    })
  }
}

const changeQuantityItemInCart = (active: string, productId: number) => {
  let cartData = JSON.parse(localStorage.getItem('cartData'))
  let quantity = 1;
  let showTotal: HTMLElement;
  let totalPrice: any;
  let index = cartData.findIndex((cart: { id: number; }) => {
    return cart.id == productId
  });
  var productSelected = products.find((product: { id: number; }) => product.id == productId);
  if (active == 'add') {
    if (index < 0) {
  ...
  if (active == 'increase') {
    if (index < 0) {
     ...
     let showMoney:any = document.querySelector(`.money-${productId}`)
  ...
  if (active == 'decrease') {
    if(cartData[index].quantity>1){
      ...
      let cartRemove = cartData.filter((item: { id: number; })=>item.id !== cartData[index].id);
      setData('cartData',cartRemove);
      document.getElementById(`${productId}`).style.display='none';
    }
  }
  if(active=='delete'){
    let cartRemove = cartData.filter((item: { id: any; })=>item.id !== cartData[index].id);
    setData('cartData',cartRemove);
    document.getElementById(`${productId}`).style.display='none';
  }
...
const addRemove = () => {
  ...
  for(let i=0; i<add.length; i++){
    add[i].addEventListener('click',function(e){
      changeQuantityItemInCart('increase', (e.target as HTMLElement).attributes['data-id'].value);
      countCart();
     })
  }
  ...
  for(let i=0; i<remove.length; i++){
    ...
    changeQuantityItemInCart('decrease', (e.target as HTMLElement).attributes['data-id'].value);
    countCart();
    emptyCart();
    })    
  }
...
const btnDelete = () => {
  ...
  for(let i=0;i<btnDelete.length;i++){
    btnDelete[i].addEventListener('click',function(e){
      changeQuantityItemInCart('delete', (e.target as HTMLElement).attributes['data-id'].value);
      countCart();
      emptyCart();
    })
  }
