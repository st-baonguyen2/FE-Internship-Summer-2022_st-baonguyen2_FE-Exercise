...
function addRemove(){
  var add = document.querySelectorAll('.fa-plus');
  ...
  var remove = document.querySelectorAll('.fa-minus')
...
function changeQuantityItemInCart(active, productId) {
  ...
  if (active == 'increase') {
    if (index < 0) {
      ...
      document.querySelector(`.money-${productId}`).innerHTML=decimalNumber(cartData[index].quantity*cartData[index].price);
    }
  }
  if (active == 'decrease') {
    if(cartData[index].quantity>1){
      ...
      var cartRemove = cartData.filter((item)=>item.id !== cartData[index].id);
...
function countCart() {
  var quantity = 0;
  var count = 0;
  var inCart = JSON.parse(localStorage.getItem('cartData'))
  for (var i = 0; i < inCart.length; i++) {
    quantity += inCart[i].quantity;
  }
